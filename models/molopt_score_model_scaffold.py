...
class ScorePosNet3D(nn.Module):
    ...
    def forward(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand,
                time_step=None, return_all=False, fix_x=False, is_scaffold_atom=None):
        """
        모델의 순전파를 수행하는 함수입니다.

        매개변수:
        - protein_pos: 단백질 원자들의 위치 (Tensor)
        - protein_v: 단백질 원자들의 특징 벡터 (Tensor)
        - batch_protein: 단백질 원자들의 배치 인덱스 (Tensor)
        - init_ligand_pos: 리간드 원자들의 초기 위치 (Tensor)
        - init_ligand_v: 리간드 원자들의 초기 원자 타입 또는 특징 벡터 (Tensor)
        - batch_ligand: 리간드 원자들의 배치 인덱스 (Tensor)
        - time_step: 확산 과정의 시간 단계 (Tensor)
        - return_all: 모든 중간 결과를 반환할지 여부 (bool)
        - fix_x: 위치를 고정할지 여부 (bool)
        - is_scaffold_atom: 스캐폴드 원자를 나타내는 마스크 (Tensor)

        반환값:
        - preds: 예측 결과를 담은 딕셔너리
        """
        ...
        # 스캐폴드 원자에 대한 마스킹 처리
        if is_scaffold_atom is not None:
            mask_non_scaffold = ~is_scaffold_atom
            # 위치 업데이트 시 스캐폴드 원자 제외
            final_ligand_pos = final_ligand_pos * mask_non_scaffold.unsqueeze(-1) + init_ligand_pos * is_scaffold_atom.unsqueeze(-1)
            # 원자 타입 업데이트 시 스캐폴드 원자 제외
            final_ligand_v = final_ligand_v * mask_non_scaffold.unsqueeze(-1) + init_ligand_v * is_scaffold_atom.unsqueeze(-1)
    ...
    @torch.no_grad()
    def sample_diffusion(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand,
                         num_steps=None, pos_only=False, center_pos_mode='ligand', return_traj=True,
                         is_scaffold_atom=None):
        """
        확산 샘플링을 수행하는 메서드입니다.
        ...
        - init_ligand_v: 리간드 원자들의 초기 원자 타입 또는 특징 벡터 (Tensor)
        - batch_ligand: 리간드 원자들의 배치 인덱스 (Tensor)
        - num_steps: 확산 과정의 단계 수 (int)
        - pos_only: 위치만 샘플링할지 여부 (bool)
        - center_pos_mode: 위치 중심화 모드 ('ligand' 또는 'protein')
        - return_traj: 중간 결과를 반환할지 여부 (bool)
        - is_scaffold_atom: 스캐폴드 원자를 나타내는 마스크 (Tensor) - 추가된 인자
        ...
        # 초기화
        ligand_pos_t = init_ligand_pos.clone()
        ...
        for t in reversed(range(num_steps)):
            time_step = torch.full((batch_ligand.max().item() + 1,), t, device=device, dtype=torch.long)

            # 모델의 forward 메서드 호출 시 is_scaffold_atom 전달
            preds = self.forward(
                protein_pos=protein_pos,
                protein_v=protein_v,
                batch_protein=batch_protein,
                init_ligand_pos=ligand_pos_t,
                init_ligand_v=ligand_v_t,
                batch_ligand=batch_ligand,
                time_step=time_step,
                is_scaffold_atom=is_scaffold_atom  # 추가된 인자
            )

            # 결과 업데이트
            ligand_pos_t = preds['pred_ligand_pos']
            ligand_v_t = preds['pred_ligand_v']

            if return_traj:
                pos_traj.append(ligand_pos_t.clone())
                v_traj.append(ligand_v_t.clone())
                if not pos_only:
                    v0_traj.append(preds['v0'].clone())
                    vt_traj.append(preds['vt'].clone())
